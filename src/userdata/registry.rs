...
impl<T> UserDataRegistry<T> {
        ...
        R: IntoLuaMulti,
    {
        ...
        Box::new(move |rawlua, nargs| unsafe {
            ...
            let args = A::from_stack_args(nargs - 1, 2, Some(&name), rawlua, state);
            ...
            match target_type {
                ...
                UserDataType::Shared(type_hints) => {
                    ...
                    try_self_arg!(borrow_userdata_scoped(state, self_index, type_id, type_hints, |ud| {
                        method(rawlua.lua(), ud, args?)?.push_into_stack_multi(rawlua, state)
                    }))
                }
        ...
        R: IntoLuaMulti,
    {
        ...
        Box::new(move |rawlua, nargs| unsafe {
            ...
            let state = rawlua.state();
            let self_ud = try_self_arg!(AnyUserData::from_stack(-nargs, rawlua, state));
            let args = A::from_stack_args(nargs - 1, 2, Some(&name), rawlua, state);
        ...
        R: IntoLuaMulti,
    {
        ...
        Box::new(move |lua, nargs| unsafe {
            let state = lua.state();
            let args = A::from_stack_args(nargs, 1, Some(&name), lua, state)?;
            function(lua.lua(), args)?.push_into_stack_multi(lua, state)
        })
    }
        ...
        R: IntoLuaMulti,
    {
        let name = get_function_name::<T>(name);
        Box::new(move |rawlua, nargs| unsafe {
            let args = match A::from_stack_args(nargs, 1, Some(&name), rawlua, rawlua.state()) {
                Ok(args) => args,
                Err(e) => return Box::pin(future::ready(Err(e))),
            };
            let lua = rawlua.lua();

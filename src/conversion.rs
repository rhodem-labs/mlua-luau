...
impl IntoLua for &Value {
    ...
    #[inline]
    unsafe fn push_into_stack(self, lua: &RawLua, state: *mut ffi::lua_State) -> Result<()> {
        lua.push_value(self, state)
    }
}
...
impl FromLua for String {
    ...
    unsafe fn from_stack(idx: c_int, lua: &RawLua, state: *mut ffi::lua_State) -> Result<Self> {
        ...
        // Fallback to default
        Self::from_lua(lua.stack_value(idx, Some(type_id), state), lua.lua())
    }
}
...
impl IntoLua for BorrowedStr<'_> {
    ...
    #[inline]
    unsafe fn push_into_stack(self, lua: &RawLua, state: *mut ffi::lua_State) -> Result<()> {
        lua.push_ref(&self.borrow.0, state);
        Ok(())
    }
}
...
impl FromLua for BorrowedStr<'_> {
    ...
    unsafe fn from_stack(idx: c_int, lua: &RawLua, state: *mut ffi::lua_State) -> Result<Self> {
        let s = String::from_stack(idx, lua, state)?;
        let BorrowedStr { buf, _lua, .. } = BorrowedStr::try_from(&s)?;
        let buf = unsafe { mem::transmute::<&str, &'static str>(buf) };
        let borrow = Cow::Owned(s);
        Ok(Self { buf, borrow, _lua })
...
impl IntoLua for RegistryKey {
    ...
    #[inline]
    unsafe fn push_into_stack(self, lua: &RawLua, state: *mut ffi::lua_State) -> Result<()> {
        <&RegistryKey>::push_into_stack(&self, lua, state)
    }
}
    ...
    T: IntoLua + AsRef<[u8]>,
{
    ...
    // Fallback to default
    lua.push_value(&T::into_lua(this, lua.lua())?, state)
}
...
impl<T: IntoLua> IntoLua for Option<T> {
    ...
    unsafe fn push_into_stack(self, lua: &RawLua, state: *mut ffi::lua_State) -> Result<()> {
        match self {
            Some(val) => val.push_into_stack(lua, state)?,
...
impl<T: FromLua> FromLua for Option<T> {
    ...
    unsafe fn from_stack(idx: c_int, lua: &RawLua, state: *mut ffi::lua_State) -> Result<Self> {
        match ffi::lua_type(state, idx) {
            ...
            _ => Ok(Some(T::from_stack(idx, lua, state)?)),
        }
    }
}
...
impl<L: IntoLua, R: IntoLua> IntoLua for Either<L, R> {
    ...
    unsafe fn push_into_stack(self, lua: &RawLua, state: *mut ffi::lua_State) -> Result<()> {
        match self {
            Either::Left(l) => l.push_into_stack(lua, state),
            Either::Right(r) => r.push_into_stack(lua, state),
        }
    }
}
...
impl<L: FromLua, R: FromLua> FromLua for Either<L, R> {
    ...
    #[inline]
    unsafe fn from_stack(idx: c_int, lua: &RawLua, state: *mut ffi::lua_State) -> Result<Self> {
        match L::from_stack(idx, lua, state) {
            Ok(l) => Ok(Either::Left(l)),
            Err(_) => match R::from_stack(idx, lua, state).map(Either::Right) {
                Ok(r) => Ok(r),
                Err(_) => {

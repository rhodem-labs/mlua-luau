...
pub fn lua_module(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let wrapped = quote! {
        ulua::require_module_feature!();

        #func

        #[no_mangle]
        unsafe extern "C-unwind" fn #ext_entrypoint_name(state: *mut ulua::lua_State) -> ::std::os::raw::c_int {
            ulua::Lua::entrypoint1(state, move |lua| {
                #skip_memory_check
                #func_name(lua)
            })
        }
...
pub fn chunk(input: TokenStream) -> TokenStream {
    ...
    let wrapped_code = quote! {{
        use ulua::{AsChunk, ChunkMode, Lua, Result, Table};
        use ::std::borrow::Cow;
        use ::std::cell::Cell;
